...
module.exports = function renderer() {
  ...
  function loadAndConvert(pvjs, callback) {
    ...
    if (pvjs.kaavioOptions.fileType === 'gpml') {
      ...
      pvjsonSource = gpml2pvjson.fixGpmlAndConvertToPvjsonSource(gpmlStringSource, pathwayMetadata)
        .map(function(result) {
          var pvjson = result.pvjson;
      ...
      pvjsonSource = RxNode.fromReadableStream(
        hyperquest(uri, {
          withCredentials: false
        })
        .pipe(JSONStream())
