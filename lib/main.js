    ...
    if (pvjs.diagramLoader.canRender(pvjs.sourceData)) {
      if (pvjs.diagramLoader.needDataConverted(pvjs.sourceData)) {
        pvjs.diagramLoader.loadAndConvert(pvjs, function(error, pvjson) {
          ...
          Kaavio.call(that, pvjs.selector, pvjs.sourceData);
          //pvjs(pvjs.selector, pvjs.sourceData);

          /*
          pvjs.on('pvjs.rendered', function(topic, callback) {
            console.log('topic');
            console.log(topic);
          });
          //*/
          /*
          var kaavio = new Kaavio(pvjs.selector, pvjs.sourceData);
          _.assign(pvjs, kaavio);
          kaavio.on(function(topic, callback) {
            console.log('topic');
            console.log(topic);
          });
          //*/
