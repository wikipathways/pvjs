    ...
    if (pvjs.diagramLoader.canRender(pvjs.kaavioOptions)) {
      if (pvjs.diagramLoader.needDataConverted(pvjs.kaavioOptions)) {
        ...
        if (pvjs.pvjsOptions.editor && pvjs.pvjsOptions.editor !== 'disabled') {
          ...
          Editor.prototype.save.bind(pvjs);
          pvjs.editor = new Editor(pvjs).bind(pvjs);
          ...
          selector.addEventListener('kaaviodatachange', function(e) {
            ...
            pvjs.editor.save({
              pvjson: e.detail.pvjson,
              pvjsonOriginal: pvjsonOriginal,
              gpmlDocJquery: pvjs.pvjsOptions.original,
              identifier: pvjs.pvjsOptions.resource.match(/WP\d+/)[0],
              version: pvjs.pvjsOptions.version
            });
          }, false);
        }
        Kaavio.call(pvjs, pvjs.selector, pvjs.kaavioOptions);
