 ...
 */
	...
	opt : {
	    ...
	    dataUrl : "http://matrixdb.ibcp.fr/matrixdbSpecifics.json",
	...
	_initialize: function (){
	    ...
	    this._setConverter (this.opt.dataUrl);	    
            this.headers =  [{
				 "sTitle" : "Interactor A"
			     }, {
...
//	    console.dir (oTable);
	 ...
	 _setConverter : function () {
	    // if (self.data != undefined) {			    
	     $.getJSON(this.opt.dataUrl, function(json) {		
			   self.data = json;		  
		       }).complete(function() { 			      
				   })
		 .error (function () {
...
//	     console.log ("data " + typeof (self.data));
...
//	     console.dir (cell);
	...
	_qtipGenerateContent :function (api, dataTable, cell) {           
		...
		var content;
		var getUrl;
		if( rawText in self.data ) {	   
		    aceClass = self.data[rawText].type;
		    content = $('<div></div>');	   
		    $(content).append(jQuery("<p id = qtipContextContent></p>"));
		    $(content).find('#qtipContextContent').append("<h4 id=qtTitle>" +  self.data[rawText].name  + "</h4>");
		    //		    $(content).log("see me?");
		    if ("chebiRef" in self.data[rawText]) {
			var chebiUrl="http://www.ebi.ac.uk/chebi/searchId.do?chebiId=" + self.data[rawText].chebiRef;
		    ...
		    if ("comments" in data[rawText]) {
			$(content).find('#qtipContextContent').append(self.data[rawText].comments);
		    }
		    if ("Stoichiometry" in data[rawText]) {
			$(content).find('#qtipContextContent').append("Stoichiometry: " + self.data[rawText].Stoichiometry);
		    }
		    if ("ebiRef" in self.data[rawText]) {
		    ...
		    if ("components" in self.data[rawText]) {
			...
			for (var i = 0; i < self.data[rawText].components.length; i++) {
			    var name = self.data[rawText].components[i];
