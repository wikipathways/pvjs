...
return}if(e.name==null){a.console.error("A `name` must be specified to run a layout");return}var d=e.name;var c=a.extension("layout",d);if(c==null){a.console.error("Can not apply layout: No such layout `%s` found; did you include its JS file?",d);return}this._private.layout=new c(b.extend({},e,{view:this._private.view,cy:this}));this._private.options.layout=e}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({notify:function(f){if(!this._private.notificationsEnabled){return}var e=this.view();
...
}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({view:function(){return this._private.view},initRenderer:function(d){var e=this;var c=a.extension("view",d.name);if(c==null){a.console.error("Can not initialise: No such view `$s` found; did you include its JS file?",d.name);return}this._private.view=new c(b.extend({},d,{cy:e,style:e._private.style,styleCalculator:{calculate:function(l,u){if(a.is.plainObject(u)){var p;if(u.customMapper!=null){p=u.customMapper.apply(l,[l.data()])
...
c.max=k[k.length-1]}else{c.min=null;c.max=null}if(n!=c.min||g!=c.max){l.continuousMapperUpdates.push({group:j.group(),element:j})}}})})(jQuery,jQuery.cytoscapeweb);(function(b,a){a.fn.core({panning:function(c){if(c!==undefined){this._private.panEnabled=c?true:false}else{return this._private.panEnabled}return this},zooming:function(c){if(c!==undefined){this._private.zoomEnabled=c?true:false}else{return this._private.zoomEnabled}return this},pan:function(d){var c=this.view().pan(d);if(c==null){this.trigger("pan");
return this}return c},panBy:function(d){var c=this.view().panBy(d);if(c==null){this.trigger("pan");return this}return c},fit:function(d){var c=this.view().fit({elements:d,zoom:true});if(c==null){this.trigger("zoom");this.trigger("pan");return this}return c},zoom:function(d){var c=this.view().zoom(d);if(c!=null){return c}else{this.trigger("zoom");return this}},center:function(c){this.view().fit({elements:c,zoom:false});this.trigger("pan");return this},centre:function(){return this.center.apply(cy,arguments)
},reset:function(){this.view().pan({x:0,y:0});this.view().zoom(1);this.trigger("zoom");this.trigger("pan");return this}})})(jQuery,jQuery.cytoscapeweb);(function(d,c){c.fn.collection=function(f,g){d.each(f,function(h,j){c.CyCollection.prototype[h]=j;c.CyElement.prototype[h]=function(){var k=this.collection();return j.apply(k,arguments)}})};var b={prefix:{nodes:"n",edges:"e"},id:{nodes:0,edges:0},generate:function(l,g,k){var f=c.is.element(g)?g._private:g;var h=f.group;var j=k!=null?k:this.prefix[h]+this.id[h];
...
},forGet:function(e){return this.isNode()}},validValue:function(e,f){return true},override:{forSet:function(f,h){var g={};g[f]=h;var e=this.cy().view().modelPoint(g);this.element()._private.position[f]=e[f]},forGet:function(f){var e=this.position(false);var g=this.cy().view().renderedPoint(e);return g[f]},forObjectGet:function(){return this.cy().view().renderedPosition(this.element())}}})});a.fn.collection({renderedStyle:function(h){var g=this.element();var f=g.cy().view();var e=f.renderedStyle(g);
...
CySelector.prototype.filter=function(collection,addLiveFunction){var self=this;if(self._private.invalid){return new $$.CyCollection(self.cy())}var selectorFunction=function(i,element){for(var j=0;j<self.length;j++){var query=self[j];if(query.group!=null&&query.group!=element._private.group){continue}var allColonSelectorsMatch=true;for(var k=0;k<query.colonSelectors.length;k++){var sel=query.colonSelectors[k];var view=self.cy().view();switch(sel){case":selected":allColonSelectorsMatch=element.selected();
break;case":unselected":allColonSelectorsMatch=!element.selected();break;case":locked":allColonSelectorsMatch=element.locked();break;case":unlocked":allColonSelectorsMatch=!element.locked();break;case":visible":allColonSelectorsMatch=view.elementIsVisible(element);break;case":hidden":allColonSelectorsMatch=!view.elementIsVisible(element);break;case":grabbed":allColonSelectorsMatch=element.grabbed();break;case":free":allColonSelectorsMatch=!element.grabbed();break;case":removed":allColonSelectorsMatch=element.removed();
...
}}return str}})(jQuery,jQuery.cytoscapeweb);(function(c,a){function b(d){c.cytoscapeweb("debug","Creating null view with options (%o)",d)}b.prototype.notify=function(d){c.cytoscapeweb("debug","Notify null view with params (%o)",d)};b.prototype.zoom=function(d){c.cytoscapeweb("debug","Zoom null view with params (%o)",d)};b.prototype.fit=function(d){c.cytoscapeweb("debug","Fit null view with params (%o)",d)};b.prototype.pan=function(d){c.cytoscapeweb("debug","Pan null view with params (%o)",d)
};b.prototype.panBy=function(d){c.cytoscapeweb("debug","Relative pan null view with params (%o)",d)};b.prototype.showElements=function(d){};b.prototype.hideElements=function(d){};b.prototype.elementIsVisible=function(d){return d._private.visible};b.prototype.renderedDimensions=function(){return{}};b.prototype.dimensions=function(){return{}};c.cytoscapeweb("view","null",b)})(jQuery,jQuery.cytoscapeweb);(function(j,m){var l={minZoom:0.001,maxZoom:1000,maxPan:-1>>>1,minPan:(-(-1>>>1)-1),selectionToPanDelay:500,dragToSelect:true,dragToPan:true,style:{selectors:{node:{fillColor:"#888",fillOpacity:1,borderColor:"#666",borderOpacity:1,opacity:1,borderWidth:0,borderStyle:"solid",height:10,width:10,shape:"ellipse",cursor:"pointer",visibility:"visible",labelValign:"top",labelHalign:"middle",labelText:{defaultValue:"",passthroughMapper:"label"},labelFillColor:"#000",labelOutlineColor:"#666",labelOutlineWidth:0,labelFontSize:"inherit",labelFontStyle:"normal",labelFontDecoration:"none",labelFontVariant:"italic",labelFontFamily:"Arial",labelFontWeight:"bold",labelOpacity:1,labelOutlineOpacity:1},edge:{lineColor:"#ccc",targetArrowColor:"#ccc",sourceArrowColor:"#ccc",targetArrowShape:"none",sourceArrowShape:"none",opacity:1,width:1,style:"solid",cursor:"pointer",visibility:"visible",labelText:{defaultValue:"",passthroughMapper:"label"},labelFillColor:"#000",labelOutlineColor:"#666",labelOutlineWidth:0,labelFontSize:"inherit",labelFontStyle:"normal",labelFontDecoration:"none",labelFontVariant:"italic",labelFontFamily:"Arial",labelFontWeight:"bold",labelOutlineOpacity:1,labelOpacity:1}},global:{panCursor:"grabbing",selectionFillColor:"#ccc",selectionOpacity:0.5,selectionBorderColor:"#888",selectionBorderWidth:1}}};
var t={};var d=function(u){j.cytoscapeweb("view","svg","linestyle",u.name,u);delete u.name};d({name:"solid",array:undefined});d({name:"dot",array:[1,5]});d({name:"longdash",array:[10,2]});d({name:"dash",array:[5,5]});var p=function(u){j.cytoscapeweb("view","svg","edgearrowshape",u.name,u);delete u.name};p({name:"triangle",svg:function(v,x,y,u,w){return v.polygon(x,[[0,1],[0.5,0],[1,1]])},centerPoint:{x:0.5,y:0.5}});p({name:"square",svg:function(v,x,y,u,w){return v.polygon(x,[[0,0],[0,1],[1,1],[1,0]])
},centerPoint:{x:0.5,y:0.5}});p({name:"circle",svg:function(v,x,y,u,w){return v.circle(x,0.5,0.5,0.5)},centerPoint:{x:0.5,y:0.5}});p({name:"diamond",svg:function(v,x,y,u,w){return v.polygon(x,[[0.5,0],[1,0.5],[0.5,1],[0,0.5]])},centerPoint:{x:0.5,y:0.5}});p({name:"tee",svg:function(v,x,y,u,w){return v.rect(x,0,0,1,0.5)},centerPoint:{x:0.5,y:0.5}});var n=function(u){j.cytoscapeweb("view","svg","nodeshape",u.name,u);delete u.name};n({name:"ellipse",svg:function(v,x,y,u,w){return v.ellipse(x,u.x,u.y,w.width,w.height)
...
}else{m.console.error("SVG view does not recognise %o as a valid visibility",u)}}function b(u){if(u!=null&&typeof u==typeof 1&&!isNaN(u)&&0<=u&&u<=1){return u}else{m.console.error("SVG view does not recognise %o as a valid percent (should be between 0 and 1)",u)}}function q(u){if(u!=null&&typeof u==typeof""&&j.Color(u)!=""){return j.Color(u).toHEX()}else{m.console.error("SVG view does not recognise %o as a valid colour",u)}}function c(u){if(u!=null&&typeof u==typeof 1&&!isNaN(u)){return u
}else{m.console.error("SVG view does not recognise %o as a valid number",u)}}function o(v){var u=j.cytoscapeweb("view","svg","nodeshape",v);if(u==null){m.console.error("SVG view does not recognise %s as a valid node shape",v)}return u}function e(v){var u=j.cytoscapeweb("view","svg","linestyle",v);if(u==null){m.console.error("SVG view does not recognise %s as a valid line style",v)}return u}function k(u){if(u=="none"||u==null){return null}return j.cytoscapeweb("view","svg","edgearrowshape",u)
...
}}}else{if(u=="grabbing"){if(j.browser.webkit){return"-webkit-grabbing"}else{if(j.browser.mozilla){return"-moz-grabbing"}else{return"move"}}}else{return u}}}function f(u){m.console.debug("Creating SVG view with options (%o)",u);this.options=j.extend({},l,u);this.setStyle(u.style);this.cy=u.cy;m.console.debug("SVG view is using style (%o)",this.style)}f.prototype.init=function(x){var w=this;this.cy=this.options.cy;var u=this.cy.container();var v=u.svg("get");this.container=u;this.svg=v;if(v!=null){u.svg("destroy")
...
}if(u.is(":removed")){m.console.debug("SVG view does not take into account removed elements when fitting");u=u.filter(":inside")}m.console.debug("Fit SVG view to view bounds");var A=this.nodesGroup.getBBox();var y,I,v,H;function E(O){if(O.height==0||O.width==0){return}var N=O.x;var z=N+O.width;var M=O.y;var w=M+O.height;if(N<y||y==null){y=N}if(z>v||v==null){v=z}if(M<I||I==null){I=M}if(w>H||H==null){H=w}}u.nodes().each(function(){var z=this.renscratch("svg").getBBox();var w=this.renscratch("svgLabel").getBBox();
...
break;default:m.console.debug("The SVG view doesn't consider the `%s` event",w.type);break}};function a(u){this.options=u;this.cy=u.cy;this.view=u.view;if(this.view.name()!="svg"){m.console.error("The SVG exporter can be used only if the SVG view is used")}}a.prototype.run=function(){return this.options.cy.container().svg("get").toSVG()};j.cytoscapeweb("view","svg",f);j.cytoscapeweb("exporter","svg",a)})(jQuery,jQuery.cytoscapeweb);(function(b,a){var c={};function d(e){this.options=b.extend(true,{},c,e)
