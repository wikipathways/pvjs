...
;(function($, $$){
	...
	$$.extension = function(){
		...
		// e.g. $$.extension("view", "svg", "nodeShape", "ellipse")
...
;(function($, $$){
	...
	$$.init = function( options ){
		...
		// e.g. $.cytoscapeweb("view", "svg", SvgRenderer);
		// e.g. $.cytoscapeweb("view", "svg", "nodeshape", "ellipse", SvgEllipseNodeShape);
...
;(function($, $$){
	...
	function CyCore( opts ){
		...
		var defaults = {
			...
			style: { // actual default style later specified by view
			}
		};
		...
		this._private = {
			...
			layout: null,
			view: null,
			notificationsEnabled: true, // whether notifications are sent to the view
			zoomEnabled: true,
			panEnabled: true
		};

		cy.initRenderer( options.view );
	...
	$$.fn.core({
		...
		json: function(params){
			...
			json.view = $$.util.copy( cy._private.options.view );
...
;(function($, $$){
	...
	$$.fn.core({
		...
		initRenderer: function( options ){
			...
			var viewProto = $$.extension("view", options.name);
			if( viewProto == null ){
				$$.console.error("Can not initialise: No such view `$s` found; did you include its JS file?", options.name);
				return;
			}
			
			this._private.view = new viewProto( $.extend({}, options, {
				cy: cy,
				style: cy._private.style,
...
;(function($, $$){
	...
	$$.fn.core({
		...
		panBy: function(params){
			var ret = this.view().panBy(params);
...
;(function($, $$){
	...
	function CyElement(cy, params, restore){
		...
		this._private = {
			...
			position: $$.util.copy( params.position ) || {}, // fields x, y, etc (could be 3d or radial coords; view decides)
			listeners: {}, // map ( type => array of function spec objects )
			group: params.group, // string; "nodes" or "edges"
			bypass: $$.util.copy( params.bypass ) || {}, // the bypass object
			style: {}, // the rendered style populated by the view
			...
			grabbed: false, // whether the element is grabbed by the mouse; view sets this privately
			...
			renscratch: {}, // object in which the view can store information
		...
		// you shouldn't and can't use this option with cy.load() since we don't have access to the view yet
		...
		if( params.renderedPosition != null ){
			this._private.position = this.cy().view().modelPoint(params.renderedPosition);
		}
...
;(function($, $$){
	...
	$$.fn.collection({
		show: function(){
			this.cy().view().showElements( this.collection() );
	...
	$$.fn.collection({
		hide: function(){
			this.cy().view().hideElements( this.collection() );
	...
	$$.fn.collection({
		visible: function(){
			return this.cy().view().elementIsVisible( this.element() );
		}
	});
...
;(function($, $$){
		...
		renderedPosition: defineAccessor({
			...
			override: {
				forSet: function( key, val ){ 
					...
					var mpos = this.cy().view().modelPoint( rpos );
					this.element()._private.position[key] = mpos[key];
				},
				forGet: function( key ){
					var mpos = this.position(false);
					var rpos = this.cy().view().renderedPoint( mpos );
					return rpos[ key ];
				},
				forObjectGet: function(){
					return this.cy().view().renderedPosition( this.element() );
				}
			}
		})
	});
	...
	$$.fn.collection({
		renderedStyle: function( property ){
			...
			var view = ele.cy().view(); // TODO remove reference after refactoring
			var rstyle = view.renderedStyle( ele );
		...
		var selectorFunction = function(i, element){
			for(var j = 0; j < self.length; j++){
				...
				for(var k = 0; k < query.colonSelectors.length; k++){
					...
					var view = self.cy().view(); // TODO remove reference after refactoring
					...
					case ":unlocked":
						...
						break;
					case ":visible":
						allColonSelectorsMatch = view.elementIsVisible(element);
						break;
					case ":hidden":
						allColonSelectorsMatch = !view.elementIsVisible(element);
						break;
...
;(function($, $$){
	...
	function NullRenderer(options){
		$.cytoscapeweb("debug", "Creating null view with options (%o)", options);
	}
	
	NullRenderer.prototype.notify = function(params){
		$.cytoscapeweb("debug", "Notify null view with params (%o)", params);
	};
	
	NullRenderer.prototype.zoom = function(params){
		$.cytoscapeweb("debug", "Zoom null view with params (%o)", params);
	};
	
	NullRenderer.prototype.fit = function(params){
		$.cytoscapeweb("debug", "Fit null view with params (%o)", params);
	};
	
	NullRenderer.prototype.pan = function(params){
		$.cytoscapeweb("debug", "Pan null view with params (%o)", params);
	};
	
	NullRenderer.prototype.panBy = function(params){
		$.cytoscapeweb("debug", "Relative pan null view with params (%o)", params);
	};
	...
	$.cytoscapeweb("view", "null", NullRenderer);
...
(function($, $$){
	...
	var registerLineStyle = function(style){
		$.cytoscapeweb("view", "svg", "linestyle", style.name, style);
		delete style.name;
	};
	...
	var registerEdgeArrowShape = function(shape){
		$.cytoscapeweb("view", "svg", "edgearrowshape", shape.name, shape);
		delete shape.name;
	};
	...
	var registerNodeShape = function(shape){
		$.cytoscapeweb("view", "svg", "nodeshape", shape.name, shape);
		delete shape.name;
	};
	...
	function visibility(v){
		if( v != null && typeof v == typeof "" && ( v == "hidden" || v == "visible" ) ){
			...
			$$.console.error("SVG view does not recognise %o as a valid visibility", v);
		}
	};
	...
	function percent(p){
		if( p != null && typeof p == typeof 1 && !isNaN(p) &&  0 <= p && p <= 1 ){
			...
			$$.console.error("SVG view does not recognise %o as a valid percent (should be between 0 and 1)", p);
		}
	}
	...
	function color(c){
		if( c != null && typeof c == typeof "" && $.Color(c) != "" ){
			...
			$$.console.error("SVG view does not recognise %o as a valid colour", c);
		}
	}
	...
	function number(n){
		if( n != null && typeof n == typeof 1 && !isNaN(n) ){
			...
			$$.console.error("SVG view does not recognise %o as a valid number", n);
		}
	}
	
	function nodeShape(name){
		var ret = $.cytoscapeweb("view", "svg", "nodeshape", name);
		
		if( ret == null ){
			$$.console.error("SVG view does not recognise %s as a valid node shape", name);
		}
	...
	function lineStyle(name){
		var ret = $.cytoscapeweb("view", "svg", "linestyle", name);
		
		if( ret == null ){
			$$.console.error("SVG view does not recognise %s as a valid line style", name);
		}
	...
	function edgeArrowShape(name){
		...
		return $.cytoscapeweb("view", "svg", "edgearrowshape", name);
	}
	...
	function labelHalign(a){
		if( a != null && typeof a == typeof "" && ( a == "left" || a == "right" || a == "middle" ) ){
			...
			$$.console.error("SVG view does not recognise %o as a valid label horizonal alignment", a);
		}	
	}
	...
	function labelValign(a){
		if( a != null && typeof a == typeof "" && ( a == "top" || a == "bottom" || a == "middle" ) ){
			...
			$$.console.error("SVG view does not recognise %o as a valid label vertical alignment", a);
		}	
	}
	...
	function SvgRenderer(options){
		$$.console.debug("Creating SVG view with options (%o)", options);
		this.options = $.extend({}, defaults, options);
		this.setStyle(options.style);
		this.cy = options.cy;
		
		
		$$.console.debug("SVG view is using style (%o)", this.style);
	}
	...
	SvgRenderer.prototype.fit = function(params){
		...
		if( elements.is(":removed") ){
			$$.console.debug("SVG view does not take into account removed elements when fitting");
			elements = elements.filter(":inside");
		}
		
		$$.console.debug("Fit SVG view to view bounds");
	...
	SvgRenderer.prototype.panBy = function(position){
		...
		$$.console.debug("Relatively pan SVG view with position (%o)", position);
	...
	SvgRenderer.prototype.pan = function(position){
		...
		$$.console.debug("Pan SVG view with position (%o)", position);
	...
	SvgRenderer.prototype.makeSvgNode = function(element){		
		...
		if( p.x == null || p.y == null ){
			$$.console.debug("SVG view is ignoring creating of node `%s` with position (%o, %o)", element.id(), p.x, p.y);
			return;
		}
		...
		element.renscratch("svg", svgDomElement);
		$$.console.debug("SVG view made node `%s` with position (%i, %i)", element.id(), p.x, p.y);
	...
	SvgRenderer.prototype.makeSvgEdge = function(element){
		...
		if( source == null || target == null ){
			$$.console.debug("SVG view is ignoring creating of edge `%s` with missing nodes");
			return;
		}
		...
		if( ps.x == null || ps.y == null || pt.x == null || pt.y == null ){
			$$.console.debug("SVG view is ignoring creating of edge `%s` with position (%o, %o, %o, %o)", element.id(), ps.x, ps.y, pt.x, pt.y);
			return;
		}
		...
		$$.console.debug("SVG view made edge `%s` with position (%i, %i, %i, %i)", element.id(), ps.x, ps.y, pt.x, pt.y);
	...
	SvgRenderer.prototype.updateNodeStyle = function(element, newStyle){
		...
		if( element.renscratch().svg == null ){
			$$.console.error("SVG view can not update style for node `%s` since it has no SVG element", element.id());
			return;
		}
		...
		$$.console.debug("SVG view collapsed mappers and updated style for node `%s` to %o", element.id(), style);
	};
	...
	SvgRenderer.prototype.updateEdgeStyle = function(element, newStyle){
		...
		if( element.renscratch().svg == null ){
			$$.console.error("SVG view can not update style for edge `%s` since it has no SVG element", element.id());
			return;
		}
		...
		$$.console.debug("SVG view collapsed mappers and updated style for edge `%s` to %o", element.id(), style);
	};
	...
	SvgRenderer.prototype.updatePosition = function(collection){
		...
		$$.console.debug("SVG view is updating node positions");
		...
		collection.nodes().each(function(i, element){
			...
			$$.console.debug("SVG view is moving node `%s` to position (%o, %o)", element.id(), p.x, p.y);
		});
	...
	SvgRenderer.prototype.positionEdges = function(edges){
		...
		edges.each(function(i, edge){
			...
			$$.console.debug("SVG view is moving edge `%s` to position (%o, %o, %o, %o)", edge.id(), ps.x, ps.y, pt.x, pt.y);
		});
	};
	...
	SvgRenderer.prototype.removeElements = function(collection, updateMappers){
		$$.console.debug("SVG view is removing elements");
	...
	SvgRenderer.prototype.notify = function(params){
		...
		$$.console.debug("Notify SVG view with params (%o)", params);
		
		if( params.type == null ){
			$$.console.error("The SVG view should be notified with a `type` field");
			return;
		}
		...
		switch( params.type ){
			...
			default:
				$$.console.debug("The SVG view doesn't consider the `%s` event", params.type);
				break;
		}
	};
	...
	function SvgExporter(options){
		...
		if( this.view.name() != "svg" ){
			$$.console.error("The SVG exporter can be used only if the SVG view is used");
		}
	}
	...
	$.cytoscapeweb("view", "svg", SvgRenderer);
