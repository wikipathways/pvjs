...
module.exports = function(pvjs1) {
  ...
  function save(pvjson) {
    ...
    var pvjsonOriginal = JSON.parse(pvjs.sourceData.normalizedPvjsonPreUpdateAsString);
    ...
    .map(function(pair) {
      var index = pair[0];
      var selectedElement = pvjson['@graph'][index];
    ...
    var standardToGpmlEntityTypeIdMappings = {
      ...
      'biopax:PhysicalEntity': 'Unknown'
    };

    _.pairs(delta)
    .filter(function(pair) {
      var value = pair[1];
      return value.hasOwnProperty('displayName') || value.hasOwnProperty('gpml:Type') ||
    ...
    .map(function(selectedPvjsElement) {
      ...
      var displayName = selectedPvjsElement.displayName;
      var datasetBridgeDbName;
      var identifier;
      var entityReferenceComponents;
      if (!!selectedPvjsElement.entityReference) {
        var entityReference = _.find(pvjson['@graph'],
            function(pvjsElement) {
              return pvjsElement.id === selectedPvjsElement.entityReference;
            });

        if (!!entityReference) {
          datasetBridgeDbName = entityReference.isDataItemIn.bridgeDbName;
      ...
      selectedElementJquery.attr('TextLabel', displayName);

      xrefElement.attr('Database', datasetBridgeDbName);
    ...
    .then(function(response) {
      ...
      pvjsonOriginal['@graph'] = pvjson['@graph'];
      pvjs.sourceData.normalizedPvjsonPreUpdateAsString =
