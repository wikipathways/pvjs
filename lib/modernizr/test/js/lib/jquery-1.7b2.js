...
function ajaxHandleResponses( s, jqXHR, responses ) {
	...
	if ( dataTypes[ 0 ] in responses ) {
		...
		for ( type in responses ) {
			if ( !dataTypes[ 0 ] || s.datas[ type + " " + dataTypes[0] ] ) {
				finalDataType = type;
				break;
			}
			if ( !firstDataType ) {
...
function ajaxConvert( s, response ) {
	...
	for( i = 1; i < length; i++ ) {
		...
		if ( i === 1 ) {
			for( key in s.datas ) {
				if( typeof key === "string" ) {
					datas[ key.toLowerCase() ] = s.datas[ key ];
				}
			}
		}
		...
		// If no auto and dataTypes are actually different
			...
			conv = datas[ conversion ] || datas[ "* " + current ];

			// If there is no direct data, search transitively
			if ( !conv ) {
				conv2 = undefined;
				for( conv1 in datas ) {
					tmp = conv1.split( " " );
					if ( tmp[ 0 ] === prev || tmp[ 0 ] === "*" ) {
						conv2 = datas[ tmp[1] + " " + current ];
						if ( conv2 ) {
							conv1 = datas[ conv1 ];
							if ( conv1 === true ) {
								conv = conv2;
			...
			// If we found no data, dispatch an error
			if ( !( conv || conv2 ) ) {
				jQuery.error( "No conversion from " + conversion.replace(" "," to ") );
			}
			// If found data is not an equivalence
			if ( conv !== true ) {
				// Convert with 1 or 2 datas accordingly
...
jQuery.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	if ( s.dataTypes[ 0 ] === "jsonp" ||
		...
		// Use data data to retrieve json after script execution
		s.datas["script json"] = function() {
