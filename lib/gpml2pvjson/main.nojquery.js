...
Gpml2Pvjson.prototype.toPvjson = function(gpmlPathwaySelection, pathwayMetadata, outerCallback) {
  var instance = this;
  console.log('gpmlPathwaySelection43');
  console.log(gpmlPathwaySelection);
  var xmlns = gpmlPathwaySelection.getAttribute('xmlns');
  ...
  var dataSource = gpmlPathwaySelection.getAttribute('Data-Source');
  if (!!dataSource) {
    pvjson.dataSource = dataSource;
  }

  var version = gpmlPathwaySelection.getAttribute('Version');
  if (!!version) {
    pvjson.version = version;
  }

  var author = gpmlPathwaySelection.getAttribute('Author');
  if (!!author) {
    pvjson.author = author;
  }

  var maintainer = gpmlPathwaySelection.getAttribute('Maintainer');
  if (!!maintainer) {
    pvjson.maintainer = maintainer;
  }

  var email = gpmlPathwaySelection.getAttribute('Email');
  if (!!email) {
    pvjson.email = email;
  }

  var lastModified = gpmlPathwaySelection.getAttribute('Last-Modified');
  if (!!lastModified) {
    pvjson.lastModified = lastModified;
  }

  var license = gpmlPathwaySelection.getAttribute('License');
  if (!!license) {
    pvjson.license = license;
  }

  var name = gpmlPathwaySelection.getAttribute('Name');
  if (!!name) {
  ...
  var bridgeDbOptions = {};
  var organism = gpmlPathwaySelection.getAttribute('Organism');
  if (!!organism) {
  ...
  pvjson.image = {
    ...
    'width': parseFloat(
        gpmlPathwaySelection.querySelector('Pathway Graphics')
        .getAttribute('BoardWidth')
    ),
    'height': parseFloat(
        gpmlPathwaySelection.querySelector('Pathway Graphics')
        .getAttribute('BoardHeight')
    )
  };
  ...
  function createSelectorObservable(selector, parentSelection) {
    ...
    var selectionList = parentSelection.querySelectorAll(selector);
  ...
  var xmlBiopaxSelection = gpmlPathwaySelection.querySelector('Biopax');
...
Gpml2Pvjson.prototype.toPvjsonSource = function(gpmlStringSource, pathwayMetadata) {
  ...
  console.log(instance);
  return instance.fixGpmlAndConvertToPvjsonSource.call(instance, gpmlStringSource, pathwayMetadata)
  .map(function(result) {
...
Gpml2Pvjson.prototype.fixBiopax = function(gpmlPathwaySelection) {
  ...
  var xmlBiopaxSelection = gpmlPathwaySelection.querySelector('Biopax');
  _.each(
      xmlBiopaxSelection.querySelectorAll('bp\\:PublicationXref'),
      function(xmlPublicationXrefSelection) {
        var publicationXrefId = xmlPublicationXrefSelection.getAttribute('rdf:id');
        xmlPublicationXrefSelection.getAttribute('rdf:id', null);
        xmlPublicationXrefSelection.getAttribute('rdf:about', publicationXrefId);
...
Gpml2Pvjson.prototype.syncGroupIdWithGraphId = function(gpmlPathwaySelection) {
  var instance = this;
  _.each(gpmlPathwaySelection.querySelectorAll('Group'), function(groupSelection) {
    var groupId = groupSelection.getAttribute('GroupId');
    groupSelection.setAttribute('GroupId', null);
    var graphId = groupSelection.getAttribute('GraphId');
    var groupedElementsSelection = _.each(
        gpmlPathwaySelection.querySelectorAll('[GroupRef=' + groupId + ']'),
        function(groupedElementSelection) {
          groupedElementSelection = $(this);
          groupedElementSelection.getAttribute('IsPartOf', graphId);
          groupedElementSelection.setAttribute('GroupRef', null);
        }
    );
  });
  return gpmlPathwaySelection;
...
Gpml2Pvjson.prototype.selectByMultipleTagNames = function(args) {
  ...
  elementTags.forEach(function(elementTag) {
    newElementsSelection = gpmlPathwaySelection.querySelectorAll(elementTag);
    if (!!newElementsSelection) {
      if (!!elementsSelection) {
        elementsSelection = elementsSelection.concat(newElementsSelection);
      } else {
...
Gpml2Pvjson.prototype.makeExplicit = function(gpmlPathwaySelection) {
  ...
  var graphicalElementsSelection = gpmlPathwaySelection.querySelectorAll(selector);
  // graphIdStub is whatever follows 'id' at the beginning of the GraphId string
  if (graphicalElementsSelection.length > 0) {
    _.filter(graphicalElementsSelection, function(graphicalElementSelection) {
      return (!!graphicalElementSelection.getAttribute('GraphId'));
    })
    .each(function(filteredResult) {
      graphId = filteredResult.getAttribute('GraphId');
    ...
    graphicalElementsSelection.filter(function() {
      ...
      filteredResult.getAttribute('GraphId', 'id' + largestGraphIdStub);
    });
    ...
    var groupCollectionSelection = _.each(gpmlPathwaySelection.querySelectorAll('Group'),
      function(groupSelection) {
        ...
        var groupStyle = groupSelection.getAttribute('Style') || 'None';
    ...
    var nodesSelection = gpmlPathwaySelection.querySelectorAll(nodesSelector);
    if (nodesSelection.length > 0) {
      var labelsSelection = gpmlPathwaySelection.find('Label');
      if (labelsSelection.length > 0) {
        _.filter(labelsSelection, function(labelSelection) {
          var graphicsSelection = labelSelection.querySelectorAll('Graphics');
          return (!graphicsSelection.getAttribute('ShapeType'));
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('ShapeType', 'None');
        });
        _.filter(labelsSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (!graphicsSelection.getAttribute('FillColor'));
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('FillColor', 'Transparent');
        });
      }

      var statesSelection = gpmlPathwaySelection.querySelectorAll('State');
      if (statesSelection.length > 0) {
        _.filter(statesSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (!graphicsSelection.getAttribute('FillColor'));
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('FillColor', 'ffffff');
        });

        _.filter(statesSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (!graphicsSelection.getAttribute('FontSize'));
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('FontSize', 10);
        });

        _.filter(statesSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (!graphicsSelection.getAttribute('Valign'));
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('Valign', 'Middle');
        });
      }

      var shapesSelection = gpmlPathwaySelection.querySelectorAll('Shape');
      if (shapesSelection.length > 0) {
        _.filter(shapesSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (!graphicsSelection.getAttribute('FillColor'));
        })
        .each(function(el) {
        ...
        _.filter(shapesSelection, function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          return (graphicsSelection.getAttribute('Rotation') === '0.0');
        })
        .each(function(el) {
          var graphicsSelection = el.querySelector('Graphics');
          graphicsSelection.setAttribute('Rotation', null);
        });

        var cellularComponentsSelection = _.each(
            shapesSelection.querySelectorAll('[Key="org.pathvisio.CellularComponentProperty"]'),
            function(cellularComponentSelection) {
              cellularComponentValue = cellularComponentSelection.getAttribute('Value');
              cellularComponentSelection.parent().setAttribute(
                  'CellularComponent', cellularComponentValue
              );
            }
        );
      }
      ...
      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (graphicsSelection.getAttribute('ShapeType') === 'Oval');
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('ShapeType', 'Ellipse');
      });

      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (!graphicsSelection.getAttribute('Padding'));
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('Padding', '0.5em');
      });
      ...
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('ShapeType', 'Rectangle');
      });

      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (!graphicsSelection.getAttribute('Color'));
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('Color', '000000');
      });

      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (!graphicsSelection.getAttribute('LineThickness'));
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('LineThickness', 1);
      });

      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (!graphicsSelection.getAttribute('ZOrder'));
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('ZOrder', 0);
      });

      _.filter(nodesSelection, function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        return (!graphicsSelection.getAttribute('Align'));
      })
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('Align', 'Center');
      });
      ...
      .each(function(el) {
        var graphicsSelection = el.querySelector('Graphics');
        graphicsSelection.setAttribute('Valign', 'Top');
      });
      ...
      trianglesSelection.each(function() {
        ...
        gpmlCenterX = parseFloat(triangleSelection.getAttribute('CenterX'));
        gpmlCenterY = parseFloat(triangleSelection.getAttribute('CenterY'));
        gpmlWidth = parseFloat(triangleSelection.getAttribute('Width'));
        gpmlHeight = parseFloat(triangleSelection.getAttribute('Height'));
        ...
        var gpmlRotation = triangleSelection.getAttribute('Rotation') || 0;
        ...
        triangleSelection.setAttribute('CenterX', correctedX + correctedWidth / 2)
        .setAttribute('CenterY', correctedY + correctedHeight / 2)
        .setAttribute('Height', correctedHeight)
        .setAttribute('Width', correctedWidth);
      });
      var arcSelection;
      var arcsSelection = shapesSelection.find('[ShapeType="Arc"]').each(function() {
        arcSelection = $(this);

        var gpmlHeight = parseFloat(arcSelection.getAttribute('Height'));
        var gpmlWidth = parseFloat(arcSelection.getAttribute('Width'));
        var correctedHeight = gpmlHeight / 2;

        // TODO refactor this code to be DRY (repeated above)
        var gpmlRotation = arcSelection.getAttribute('Rotation') || 0;
        ...
        gpmlCenterX = parseFloat(arcSelection.getAttribute('CenterX'));
        gpmlCenterY = parseFloat(arcSelection.getAttribute('CenterY'));
        ...
        arcSelection.setAttribute('CenterX', correctedCenterX)
        .setAttribute('CenterY', correctedCenterY);
      });
      ...
      var pentagonsSelection = shapesSelection.find('[ShapeType="Pentagon"]').each(function() {
        ...
        gpmlWidth = parseFloat(pentagonSelection.getAttribute('Width'));
        gpmlHeight = parseFloat(pentagonSelection.getAttribute('Height'));
        gpmlCenterX = parseFloat(pentagonSelection.getAttribute('CenterX'));
        pentagonSelection.setAttribute('CenterX', gpmlCenterX +
            gpmlWidth * (1 - pentagonXScaleFactor) / 2)
        .setAttribute('Width', gpmlWidth * pentagonXScaleFactor)
        .setAttribute('Height', gpmlHeight * pentagonYScaleFactor);
      });
      ...
      var hexagonsSelection = shapesSelection.find('[ShapeType="Hexagon"]').each(function() {
        ...
        gpmlHeight = parseFloat(hexagonSelection.getAttribute('Height'));
        hexagonSelection.setAttribute('Height', gpmlHeight * hexagonYScaleFactor);
      });
      ...
      if (dataNodesSelection.length > 0) {
        dataNodesSelection.filter(function() {
          return (!$(this).find('Graphics').setAttribute('FillColor'));
        }).each(function() {
          $(this).find('Graphics').setAttribute('FillColor', 'ffffff');
        });
      }
    ...
    var doubleLinesSelection = gpmlPathwaySelection.find(
      ...
      .each(function() {
        $(this).parent().find('Graphics').setAttribute('LineStyle', 'Double');
      });
    ...
    if (edgesSelection.length > 0) {
      edgesSelection.each(function() {
        $(this).find('Graphics').setAttribute('FillColor', 'Transparent');
      });
      edgesSelection.filter(function() {
        var graphicsSelection = $(this).find('Graphics');
        return (!graphicsSelection.setAttribute('ConnectorType'));
      }).each(function(d, i) {
        $(this).find('Graphics').setAttribute('ConnectorType', 'Straight');
      });
      edgesSelection.filter(function() {
        return (!$(this).find('Graphics').getAttribute('Color'));
      }).each(function(d, i) {
        $(this).find('Graphics').setAttribute('Color', '000000');
      });
      ...
      if (anchorsSelection.length > 0) {
        anchorsSelection.each(function() {
          ...
          var shapeTypeValue = anchorSelection.getAttribute('Shape') || 'None';
          var positionValue = anchorSelection.getAttribute('Position');
          ...
          if (typeof(document) !== 'undefined' && !!document && !!document.createElementNS) {
            ...
            graphicsElement.setAttribute(
                'FillColor', parentGraphicsSelection.getAttribute('Color'));
            ...
            anchorSelection.append('<Graphics Position="' + positionValue + '" ShapeType="' +
              ...
              parentGraphicsSelection.getAttribute('Color') + '"></Graphics>');
          }

          anchorSelection.getAttribute('Position', null);
          anchorSelection.getAttribute('Shape', null);
        ...
        anchorsSelection.filter(function() {
          ...
          if (graphicsSelection.length > 0) {
            result = graphicsSelection.getAttribute('ShapeType') === 'Circle';
          }
          return result;
        }).each(function(d, i) {
          var graphicsSelection = $(this).find('Graphics');
          graphicsSelection.setAttribute('ShapeType', 'Ellipse');
          graphicsSelection.setAttribute('Width', 8);
          graphicsSelection.setAttribute('Height', 8);
        });
        anchorsSelection.filter(function() {
          var graphicsSelection = $(this).find('Graphics');
          var result = false;
          if (graphicsSelection.length > 0) {
            result = graphicsSelection.getAttribute('ShapeType') === 'None';
          }
          return result;
        }).each(function(d, i) {
          var graphicsSelection = $(this).find('Graphics');
          graphicsSelection.setAttribute('Width', 4);
          graphicsSelection.setAttribute('Height', 4);
        });
      }
    }
  }
...
(function() {
  if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    ...
    window.Gpml2Pvjson = Gpml2Pvjson;
  }
