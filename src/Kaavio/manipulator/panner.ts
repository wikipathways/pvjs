...
export class Panner {
    ...
    private animatePan(coordinates: {x: number, y: number}): Promise<any> {
        const duration = 300;
        const panStep = 1;
     ...
     * @param animate
     */
    pan(coordinates: {x: number, y: number}, animate = true): Promise<any> {
        if(animate)
     ...
     */
    panTo(node_id: string | string[], animate = true): Promise<any> {
        ...
        return this.panZoom.isUpdating$
            ...
            .switchMap(() => {
                ...
                return Observable.fromPromise(this.pan(coordinates, animate));
            })
            .toPromise();
    }
