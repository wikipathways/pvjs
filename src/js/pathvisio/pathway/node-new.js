...
pathvisio.pathway.node = function(){
  ...
  var gpmlToSvgShapeMappings = { 'rectangle':'rect', 'rounded-rectangle':'rect', 'oval':'ellipse'};
      ...
      rawJsonNodes.forEach(function(element, index, array) {
        ...
        if ((!(element.graphics.hasOwnProperty("shapeType")))) {
          if (element.elementType === 'data-node') {
            element.shape.name = "rect";
          }
          else {
            element.shape.name = 'use';
            element.shape.symbolName = 'none';
          }
        }
        else {
          element.shape.name = caseConverter.paramCase(element.graphics.shapeType);
          if (!gpmlToSvgShapeMappings[element.shape.name]) {
            element.shape.symbolName = element.shape.name;
            element.shape.name = 'use';
          }
          else {
  ...
  function drawAll(svg, pathway) {
    ...
    var nodes = nodesContainer.each(function(d, i) {
      var node = d3.select(this).append(d.shape.name);
      if (d.shape.name === 'use') {
        symbol = svg.select('symbol#' + d.shape.symbolName)
        if (symbol.length === 1) {

          // d3 bug strips 'xlink' so need to say 'xlink:xlink';
          
          node.attr("xlink:xlink:href", function (d) {return "#" + d.shape.symbolName;});
        }
        else {
          node.attr("xlink:xlink:href", "#none");
          console.warn('Pathvisio.js does not have access to the requested symbol: ' + pathway.nodes[0].symbolName + '.');
        }
      }
      else {
        if (d.shape.name === 'rect') {
          node.attr("x", 0)
          .attr("y", 0)
      ...
      node.attr("id", function (d) {return 'node-' + d.graphId;})
      ...
      .attr("style", function (d) {
        ...
        if (d.hasOwnProperty('strokeStyle')) {
          ...
          if (d.strokeStyle === 'double') {
            ...
            .attr("xlink:xlink:href", function (d) {return "#none";})
