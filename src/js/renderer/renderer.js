pathvisiojs.renderer = function(){
  ...
  function generateDiagramId(uri){
    ...
    var id = ('id-' + uri).replace(/[^A-Za-z0-9-]/g, '').toLowerCase();
    return id;
  }
  ...
  var sourceDataFileTypes = {
    gpml:{
      supportedRendererFormats:[
      'svg' //in the future, could add canvas support
      ],
      imageGeneratedDynamically: true
    },
    biopax:{
      supportedRendererFormats:[
      'svg' //biopax is not currently supported
      ],
      imageGeneratedDynamically: true
    },
    pdf:{ //not supported now. this would be future. we might use pdf.js or we could just try using an embed or object tag.
      ...
      imageGeneratedDynamically: false
    },
    png:{
      supportedRendererFormats:[
        'img'
  ...
  function loadHtmlTemplate(userSpecifiedContainer, callback) {
    ...
    var pathvisiojsContainer = userSpecifiedContainer.select('#pathvisiojs-container');
    console.log('pathvisiojsContainer');
    console.log(pathvisiojsContainer);
    var containerBoundingClientRect = pathvisiojsContainer[0][0].getBoundingClientRect();
    console.log('containerBoundingClientRect');
    console.log(containerBoundingClientRect.width);
    pathvisiojsContainer.style('width', containerBoundingClientRect.width + 'px');

    var diagramContainer = userSpecifiedContainer.select('#diagram-container')
    .style('height', containerBoundingClientRect.height + 'px');
    console.log('diagramContainer[0][0]');
    console.log(diagramContainer[0][0]);
    callback(diagramContainer, containerBoundingClientRect);
  }

  // *******************************************
  // this function takes input source data and renders it as a diagram
  // *******************************************
  function load(args, callbackOutside) {
    ...
    async.waterfall([
      function(callback){ // this could be in parallel
        ...
        // Load HTML template
        // ********************************************
        var htmlTemplate = loadHtmlTemplate(userSpecifiedContainer, function(thisPathwayContainer, containerBoundingClientRect) {
          diagramContainer = thisPathwayContainer;
          callback(null, containerBoundingClientRect);
        });
      },
      function(containerBoundingClientRect, callback){
        // ********************************************
        // Add loading gif
        // ********************************************
        ...
        // ********************************************
        // Get desired dimensions for diagram
        // ********************************************
        ...
        // Get desired dimensions for diagram
        // ********************************************
        var containerWidth = containerBoundingClientRect.width - 3; //account for space for pan/zoom controls,
        var containerHeight = containerBoundingClientRect.height - 3; //account for space for search field;
      ...
      function(thisContainerWidth, thisContainerHeight, renderableSourceDataElement, callback){
        var svg;
        containerWidth = thisContainerWidth;
        ...
        if (sourceDataFileTypes[renderableSourceDataElement.fileType].imageGeneratedDynamically) {
          ...
          loadDiagramArgs.containerWidth = thisContainerWidth;
          loadDiagramArgs.containerHeight = thisContainerHeight;
          loadDiagramArgs.fitToContainer = fitToContainer;
          loadDiagramArgs.highlights = highlights;

          userSpecifiedContainerSelector.select('#typeahead').remove();
          userSpecifiedContainerSelector.select('#pan-zoom-control').remove();
