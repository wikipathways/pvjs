...
module.exports = function(){
  function RendererSvg(pvjs, options) {
    this.pvjs = pvjs
    ...
    this.crossPlatformTextInstance.init({
      targetSelector: '#' + options.id
    });
  }

  RendererSvg.prototype.render = function(data) {
    if (data.shape !== 'none') {
  ...
  RendererSvg.prototype.renderShape = function(data) {
    var shapeName = Strcase.camelCase(data.shape)
      , self = this

    // TODO move this checking into plugin
    if (!this.crossPlatformShapesInstance.hasOwnProperty(shapeName)) {
      ...
      if (crossPlatformShapesInstance.hasOwnProperty(shapeName)) {
    ...
    var node = this.crossPlatformShapesInstance[shapeName](data)
      , $node = d3.select(node)

    if (!!data.datasourceReference) {
      var notDragged = true
        , dr = data.datasourceReference
      ...
      .on("mouseup", function(d,i) {
        if (notDragged) {
          XRef.render(self.pvjs, dr.organism, dr.id, dr.database, data.textContent, data.dataNodeType);
        }
      });
    }
  }

  RendererSvg.prototype.renderText = function(data) {
    var node = this.crossPlatformTextInstance.render(data)

    d3.select(node).attr('pointer-events', 'none');
  }

  function init(pvjs, options){
    return new RendererSvg(pvjs, options)
  }
