...
export class CustomElement extends HTMLElement {
  ...
  arraySetter(name, val) {
    const toSet = val.reduce(
      (acc, el, i) => `${acc}${el}${i === val.length - 1 ? "" : ","}`,
      ""
    );
  ...
  highlightOff(entityId: string) {
    this.highlightedEntities = this.highlightedEntities.filter(
      single => single.entityId !== entityId
    );
    this.renderPvjs();
  }

  toggleHighlight(entityId: string, color: string) {
    const isHighlighted = !!this.highlightedEntities.filter(
      single => single.entityId === entityId
    )[0];
  ...
  resetHighlighted(exclude = []) {
    this.highlightedEntities = this.highlightedEntities.filter(
      single => exclude.indexOf(single.entityId) > -1
    );
    this.renderPvjs();
  }
  ...
  show(entityId: string) {
    this.hiddenEntities = this.hiddenEntities.filter(
      single => single !== entityId
    );
    this.renderPvjs();
  }
  ...
  zoomOn(entityId: string | string[]) {
    let toZoom;
    if (Array.isArray(entityId)) toZoom = entityId;
    else toZoom = [entityId];
  ...
  panTo(entityId: string | string[]) {
    let toPan;
    if (!Array.isArray(entityId)) toPan = [entityId];
    else toPan = entityId;
